...
export default function AdminDashboard() {
  ...
  const style = {
    ...
    badge: (status) => {
      ...
      if (status === "Processing" || status === false) {
        bg = "#fff1f2";
        color = "#be123c";
      }
      return {
    ...
    actionBtn: (type) => ({
      ...
      fontWeight: "700",
      cursor: "pointer",
      background: type === "ship" ? "#eff6ff" : "#ecfdf5",
      color: type === "ship" ? "#2563eb" : "#16a34a",
      border: type === "ship" ? "1px solid #bfdbfe" : "1px solid #bbf7d0",
    }),
  };

  return (
    <div style={style.wrapper} className="am-wrapper">
      <style>{`
        .am-wrapper * { box-sizing: border-box; }
        ...
        .am-product-row { }
        .am-row-main { display: flex; align-items: center; flex: 1; min-width: 0; }
        .am-row-meta { display: flex; align-items: center; gap: 16px; }

        .am-form-row { display: flex; gap: 12px; }
        .am-btn-row { display: flex; gap: 10px; margin-top: 16px; }

        .am-kpi-card { }

        @media (max-width: 768px) {
          .am-header { flex-direction: column; align-items: stretch !important; }
          .am-search-form { width: 100%; }
          .am-search-input { width: 100% !important; }
          .am-tabs { flex-wrap: wrap; }
          .am-tabs button { flex: 1 1 auto; }
        }

        @media (max-width: 600px) {
          .am-wrapper { padding: 16px !important; }
          .am-workspace { grid-template-columns: 1fr !important; gap: 16px !important; }
        }

        @media (max-width: 480px) {
          .am-grid3 { grid-template-columns: 1fr !important; }
          .am-kpi-card { padding: 16px !important; }
          .am-product-row { flex-wrap: wrap; row-gap: 10px; }
          .am-row-main { flex: 1 1 100%; }
          .am-row-meta { width: 100%; justify-content: space-between; }
        }

        @media (max-width: 420px) {
          .am-form-row { flex-direction: column; gap: 0; }
        }
      ...
      <div style={style.header} className="am-header">
        <h1 style={style.title}>
          Mechora <span style={style.subtitle}>Control Hub</span>
        </h1>

        <form
          onSubmit={handleBarcodeRegistration}
          className="am-search-form"
        ...
        >
          ...
          <button
            type="submit"
            style={{ ...style.btnSubmit, padding: "10px 14px", flex: "none" }}
          >
            Find SKU
          </button>
        </form>

        <div style={style.tabContainer} className="am-tabs">
          <button
            onClick={() => setActiveTab("inventory")}
            style={style.tabBtn(activeTab === "inventory")}
          >
            📦 Inventory
          </button>
          <button
            onClick={() => setActiveTab("orders")}
            style={style.tabBtn(activeTab === "orders")}
          >
            📋 Orders Queue
          </button>
          <button
            onClick={() => setActiveTab("profile")}
            style={style.tabBtn(activeTab === "profile")}
          >
            👤 Fleet Coordinates
          </button>
        </div>
      ...
      <div style={style.grid3} className="am-grid3">
        <div style={style.kpiCard} className="am-kpi-card">
      ...
      {activeTab === "inventory" && (
        <div style={style.workspace} className="am-workspace">
            ...
            <div style={{ maxHeight: "540px", overflowY: "auto" }}>
              {products.map((product) => (
                <div
                  ...
                  style={style.productRow(selectedProduct?.id === product.id)}
                  className="am-product-row"
                >
          ...
          <div style={style.kpiCard} className="am-kpi-card">
            ...
            >
              {isEditMode
                ? "⚡ Edit Catalog Entry"
                : "➕ Inventory Console: Add New Product"}
            </h3>
            ...
            {invMessage.text && (
              <div
                style={{
                  ...
                  marginBottom: "12px",
                  background:
                    invMessage.type === "success" ? "#edfdf5" : "#fff1f2",
                  color: invMessage.type === "success" ? "#047857" : "#be123c",
                }}
              >
                {invMessage.text}
              </div>
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setEditForm({ ...editForm, salt: e.target.value })
                }
                style={{
                  ...style.input,
              ...
              <div className="am-btn-row">
                {isEditMode ? (
                  <>
                    ...
                    <button
                      type="submit"
                      disabled={invLoading || uploadingImage}
                      style={style.btnSubmit}
                    >
                      {invLoading
                        ? "Saving Changes..."
                        : "Save Row Alterations"}
                    </button>
                  </>
                ) : (
                  <>
          ...
          {orderMessage.text && (
            <div
              style={{
                ...
                fontSize: "13px",
                background:
                  orderMessage.type === "success" ? "#ecfdf5" : "#fff1f2",
                color: orderMessage.type === "success" ? "#047857" : "#be123c",
              }}
            >
              {orderMessage.text}
            </div>
          ...
          <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
            ...
            >
              <thead>
                ...
                >
                  <th style={{ padding: "14px 20px", color: "#64748b" }}>
                    Order ID
                  </th>
                  <th style={{ padding: "14px 20px", color: "#64748b" }}>
                    Customer Base
                  ...
                  orders.map((order) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "14px 20px", color: "#334155" }}>
                        {order.user_id
                          ? `Client: ${String(order.user_id).slice(0, 8)}`
                          : "Guest Account"}
                      </td>
                      <td
                        style={{
                      ...
                      <td style={{ padding: "14px 20px" }}>
                        <span style={style.badge(order.status)}>
                        ...
                        >
                          {order.status !== "Shipped" &&
                            order.status !== "Delivered" && (
                              <button
                                onClick={() =>
                                  alterOrderShipmentLifecycle(
                                    order.id,
                                    "Shipped",
                                  )
                                }
                                style={style.actionBtn("ship")}
                              >
                                🚀 Ship Out
                              </button>
                            )}
      ...
      {activeTab === "profile" && (
        <div
          style={{ ...style.kpiCard, maxWidth: "560px", margin: "0 auto" }}
        ...
        >
          ...
          >
            👤 System Coordinates & Hub Profile
          </h3>
          ...
          {profileMessage.text && (
            <div
              style={{
                ...
                marginBottom: "12px",
                background:
                  profileMessage.type === "success" ? "#ecfdf5" : "#fff1f2",
                color:
                  profileMessage.type === "success" ? "#047857" : "#be123c",
              }}
            >
              {profileMessage.text}
            </div>
          ...
          <form onSubmit={handleUpdateProfile}>
            ...
            <label
              style={{ fontSize: "11px", color: "#64748b", fontWeight: "600" }}
            >
              Hub Coordinates / Dispatch Address
            </label>
            <textarea
            ...
            <button
              ...
              style={{ ...style.btnSubmit, width: "100%" }}
            >
              {profileLoading
                ? "Syncing Grid Configuration..."
                : "Rewrite Profile Master File"}
            </button>
          </form>
        </div>
      )}
