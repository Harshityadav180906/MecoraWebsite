...
export default function OrdersView({ role, orders, fetchOrders }) {
  ...
  return (
    <>
      <style>{`
        .ov-wrapper { height: calc(100vh - 5rem); display: flex; flex-direction: column; box-sizing: border-box; }
        .ov-wrapper * { box-sizing: border-box; }

        .ov-split { display: grid; grid-template-columns: 380px 1fr; gap: 1.5rem; flex-grow: 1; min-height: 0; }
        .ov-master-list { overflow-y: auto; display: flex; flex-direction: column; gap: 0.75rem; padding-right: 0.5rem; }
        .ov-detail-col { overflow-y: auto; }
        .ov-detail-grid { display: grid; grid-template-columns: 1fr 1.2fr; gap: 1.5rem; height: 100%; }

        .ov-order-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; gap: 0.5rem; flex-wrap: wrap; }
        .ov-invoice-header { display: flex; justify-content: space-between; margin-bottom: 2rem; gap: 1rem; flex-wrap: wrap; }
        .ov-receipt-row { display: flex; justify-content: space-between; background: #f8fafc; padding: 0.75rem 1rem; border-radius: 8px; font-size: 0.95rem; gap: 0.75rem; flex-wrap: wrap; }
        .ov-total-row { display: flex; justify-content: space-between; align-items: center; gap: 0.75rem; flex-wrap: wrap; }

        @media (max-width: 860px) {
          .ov-wrapper { height: auto; min-height: 0; }
          .ov-split { grid-template-columns: 1fr; gap: 1rem; }
          .ov-master-list { overflow-y: auto; max-height: 320px; padding-right: 0.25rem; }
          .ov-detail-col { overflow-y: visible; }
          .ov-detail-grid { grid-template-columns: 1fr; height: auto; }
          .ov-title { font-size: 1.35rem !important; }
          .ov-empty-card { padding: 2.5rem 1.25rem !important; }
        }

        @media (max-width: 480px) {
          .ov-invoice-header { flex-direction: column; }
          .ov-invoice-meta { text-align: left !important; }
          .ov-total-amount { font-size: 1.4rem !important; }
        }
      `}</style>

      <div className="ov-wrapper">
        <div style={{ marginBottom: '1.5rem' }}>
          <h2 className="ov-title" style={{ fontSize: '1.75rem', fontWeight: '700' }}>
        ...
        {orders.length === 0 ? (
          <div className="card ov-empty-card" style={{ textAlign: 'center', padding: '4rem' }}>
              ...
              {selectedOrder ? (
                <div className="ov-detail-grid">
                  ...
                  <div className="card" style={{ background: '#ffffff', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <div>
                      <div className="ov-invoice-header">
                        <div>
                        ...
                        <div className="ov-invoice-meta" style={{ textAlign: 'right', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {selectedOrder.items?.map((item, idx) => (
                          <div key={idx} className="ov-receipt-row">
                            <div>
                ...
                <div className="card ov-empty-card" style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>
